...
export class UserVehicleService {
  ...
  private async upsertAutoReminder(
    ...
    // Find existing auto-generated reminder for this source
    ...
    if (existing) {
      ...
      await this.prisma.reminder.create({
        data: {
          ...
          title,
          dueDate: reminderDate,
          type: sourceType.toString(), // root-cause fix: sourceType is enum, type is String
          status: 'open',
          sourceId,
          sourceType,
          autoGenerated: true,
  ...
  async exportDocumentsZip(id: string) {
    const vehicle = await this.prisma.userVehicle.findUnique({
      where: { id },
      include: {
        user: { select: { plan: true } },
