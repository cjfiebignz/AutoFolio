    ...
    <AccountPageShell title={title} subtext={subtext}>
      <div className="space-y-8">
        <nav className="flex items-center justify-between px-1">
          <Link 
            href="/account/legal"
            className="group flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-white/30 hover:text-white transition-colors"
          >
        ...
        <div className="rounded-[40px] border border-white/5 bg-white/[0.02] p-8 sm:p-12 shadow-2xl backdrop-blur-md">
          <div className="space-y-12">
            {children}
          </div>
          
          <div className="mt-16 pt-8 border-t border-white/5">
            <p className="text-[10px] font-medium leading-relaxed text-white/20 italic">
...
export function LegalSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-bold uppercase tracking-tight text-white italic">{title}</h2>
      <div className="text-sm sm:text-base text-white/60 leading-relaxed space-y-4">
        {children}
      </div>
    </div>
  );
