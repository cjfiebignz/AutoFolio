...
export function PremiumUpgradeModal({ 
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 sm:p-6">
      ...
      {/* Modal Content */}
      <div className="relative w-full max-w-lg overflow-hidden rounded-[40px] border border-white/10 bg-[#0b0b0c] shadow-2xl transition-all">
        {/* Animated Background Elements - lowered z-index */}
        <div className="absolute -right-20 -top-20 h-64 w-64 rounded-full bg-white/5 blur-3xl animate-pulse z-0" />
        ...
        <div className="px-8 pb-10 sm:px-10 sm:pb-12 text-center space-y-8 relative z-10">
          <div className="space-y-3">
            <h3 className="text-3xl font-black italic tracking-tighter text-white uppercase leading-tight">
              {title}
            </h3>
            <p className="mx-auto max-w-xs text-sm font-medium leading-relaxed text-white/40">
              {message}
            </p>
          </div>

          {/* Features List */}
          <div className="grid grid-cols-1 gap-4 text-left bg-white/[0.02] border border-white/5 rounded-3xl p-6 sm:p-8">
          ...
          <div className="flex flex-col gap-3">
            {primaryAction?.href ? (
              <Link
                ...
                onClick={onClose}
                className="flex h-14 items-center justify-center gap-3 rounded-2xl bg-white px-10 text-xs font-black uppercase tracking-widest text-black transition-all hover:bg-white/90 active:scale-[0.98] shadow-xl"
              >
                {primaryAction.label}
            ...
            {secondaryActionLabel && (
              <button
                type="button"
                onClick={onClose}
                className="h-14 text-[10px] font-black uppercase tracking-widest text-white/20 hover:text-white transition-colors"
              >
                {secondaryActionLabel}
              </button>
            )}
        ...
        {/* Footer info */}
        <div className="relative z-10 p-6 bg-white/[0.01] border-t border-white/5 text-center shrink-0">
...
function FeatureItem({ label }: { label: string }) {
  return (
    <div className="flex items-center gap-3">
      <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-green-500/10 text-green-500">
