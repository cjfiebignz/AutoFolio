...
export function MaintenanceStatusBadge({ status, size = 'md', className = '' }: MaintenanceStatusBadgeProps) {
  const config = {
    up_to_date: {
      ...
      styles: 'bg-green-500/10 text-green-700 dark:text-green-400 border-green-500/20 shadow-[0_0_15px_rgba(34,197,94,0.1)]',
      dot: 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]'
    },
    due_soon: {
      label: 'Due Soon',
      icon: <AlertTriangle size={size === 'sm' ? 10 : 14} />,
      styles: 'bg-yellow-500/10 text-yellow-700 dark:text-yellow-400 border-yellow-500/20 shadow-[0_0_15px_rgba(234,179,8,0.1)]',
      dot: 'bg-yellow-500 shadow-[0_0_8px_rgba(234,179,8,0.6)]'
    },
    overdue: {
      label: 'Overdue',
      icon: <AlertCircle size={size === 'sm' ? 10 : 14} />,
      styles: 'bg-red-500/10 text-red-700 dark:text-red-400 border-red-500/20 shadow-[0_0_15px_rgba(239,68,68,0.1)]',
      dot: 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.6)]'
    },
    insufficient_data: {
      label: 'Setup Required',
      icon: <HelpCircle size={size === 'sm' ? 10 : 14} />,
      styles: 'bg-card-overlay text-muted border-border-strong/50 dark:border-border-subtle',
