    ...
    <div className="mt-6 border-t border-subtle pt-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className="flex h-5 w-5 items-center justify-center rounded-md bg-foreground/5 text-muted">
        ...
        <button
          ...
          className="group flex h-8 items-center justify-center gap-2 rounded-xl bg-foreground/5 pl-2.5 pr-3.5 text-[9px] font-black uppercase tracking-widest text-muted transition-all hover:bg-foreground/10 hover:text-foreground disabled:opacity-50"
        >
          {isUploading ? (
            <Loader2 size={12} className="animate-spin" />
          ) : (
            <div className="flex h-4 w-4 items-center justify-center rounded-md bg-foreground/5 text-dim group-hover:bg-foreground/10 group-hover:text-muted transition-colors">
      ...
      {localAttachments.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-10 rounded-[24px] bg-foreground/[0.01] border border-dashed border-subtle">
           <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-foreground/5 text-dim">
             <File size={20} strokeWidth={1.5} />
           </div>
           <p className="text-[10px] font-bold text-dim uppercase tracking-widest">No files attached</p>
        </div>
      ) : (
        <div className="grid gap-2">
          {localAttachments.map((attachment) => {
            ...
            return (
              <div 
                key={attachment.id} 
                className={`group flex items-center justify-between rounded-2xl bg-card-overlay border border-subtle p-2 pr-3 transition-all hover:bg-card-overlay-hover hover:border-border-strong ${isRowDeleting ? 'opacity-50 grayscale' : ''} ${isConfirming ? 'border-red-500/20 bg-red-500/5' : ''}`}
              >
                <div className="flex items-center gap-3 min-w-0">
                  {isImage ? (
                    <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-lg bg-foreground/5 border border-subtle group-hover:border-border-strong transition-colors">
                      <img 
                    ...
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-foreground/5 text-dim group-hover:text-muted border border-subtle group-hover:border-border-strong transition-all">
                  ...
                  <div className="min-w-0 flex flex-col justify-center">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-[8px] font-black text-dim uppercase tracking-wider bg-foreground/5 px-1.5 py-0.5 rounded-md">
                ...
                <div className="flex items-center gap-1.5 ml-4">
                  {!isConfirming && (
                    <a
                      ...
                      className="flex h-8 w-8 items-center justify-center rounded-xl bg-foreground/5 text-dim hover:bg-foreground/10 hover:text-muted transition-all"
                  ...
                  {isConfirming && (
                    <button
                      ...
                      className="flex h-8 w-8 items-center justify-center rounded-xl bg-foreground/10 text-muted hover:text-foreground transition-all"
                      title="Cancel"
                    >
