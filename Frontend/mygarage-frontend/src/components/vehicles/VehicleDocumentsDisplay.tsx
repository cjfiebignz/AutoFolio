...
import { 
  ...
  FileImage, CheckCircle2, X, Loader2, Archive
} from 'lucide-react';
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between px-1">
          <div className="space-y-1.5">
            <h2 className="text-3xl font-black italic tracking-tighter text-foreground uppercase leading-none">Vault</h2>
            <p className="text-[10px] font-black uppercase tracking-[0.25em] text-accent opacity-60 dark:text-blue-400/50">Encrypted Document Storage</p>
          </div>
          <Link 
        ...
        {selectedIds.size > 0 && (
          <div className="flex flex-wrap items-center gap-2 px-1 animate-in fade-in slide-in-from-top-2 duration-500">
            <button
              ...
              className="flex h-9 items-center justify-center gap-2 rounded-xl border border-subtle bg-card-overlay px-3 text-[10px] font-black uppercase tracking-widest text-muted transition-all hover:bg-card-overlay-hover active:scale-95"
            >
            ...
            <button
              ...
              className="flex h-9 items-center justify-center gap-2 rounded-xl border border-accent/20 bg-accent/5 px-4 text-[10px] font-black uppercase tracking-widest text-accent transition-all hover:bg-accent/10 active:scale-95 shadow-lg shadow-accent/5 disabled:opacity-50"
            >
  ...
  return (
    <div 
      ...
      className={`group relative overflow-hidden rounded-[28px] border transition-all duration-500 cursor-pointer ${
        isSelected 
          ? 'border-accent/30 bg-accent/[0.03] shadow-lg ring-1 ring-accent/20' 
      ...
      <div className="flex flex-col">
        <div className="flex items-center gap-4 p-4 sm:p-5">
          ...
          <div className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border transition-all ${
            isSelected 
              ? 'bg-accent border-accent/40 text-background' 
              : 'border-border-strong bg-foreground/5 text-transparent group-hover:border-accent/40'
          }`}>
          ...
          <div className="min-w-0 flex-1 space-y-0.5">
            <p className="text-[8px] font-black uppercase tracking-[0.25em] text-dim">{doc.category}</p>
            <h3 className="text-sm font-bold text-foreground uppercase italic tracking-tight truncate">{doc.title}</h3>
            <p className="text-[9px] font-bold text-muted uppercase tracking-widest">{doc.date}</p>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0" onClick={e => e.stopPropagation()}>
            {doc.fileUrl && (
              <>
                <a 
                  ...
                  className="flex h-9 w-9 items-center justify-center rounded-xl bg-card-overlay text-muted hover:bg-card-overlay-hover hover:text-foreground transition-all border border-subtle"
                  title="View Document"
                >
                  <Eye size={14} />
                </a>
            ...
            {confirmDelete && (
              <button 
                ...
                className="h-9 w-9 flex items-center justify-center rounded-xl bg-card-overlay text-muted hover:text-foreground transition-colors"
              >
                <X size={14} />
              </button>
            )}
...
function EmptyDocumentState({ vehicleId }: { vehicleId: string }) {
  return (
    <div className="group relative flex flex-col items-center justify-center rounded-[40px] border border-dashed border-border-strong bg-card-overlay py-32 text-center transition-all hover:bg-card-overlay-hover">
      ...
      <h3 className="text-xl font-bold text-muted uppercase italic tracking-tight">Vault is empty</h3>
